...
Materia: *Laboratorio de Implementación III*
...
"""h) **Redes Neuronales Recurrentes Bidireccionales (GRU)**"""

# #GRU_11.5_stdscaler_tn_BI100755025u_lossTFE_seq3_lr0.0001

# # Cargar el dataset
# data = monthly_data_filtrado
# data['periodo'] = pd.to_datetime(data['periodo'])
# data.sort_values(by=['product_id', 'periodo'], inplace=True)

# # Modificar la definición de features para incluir 'tn'
# features = [col for col in data.columns if col not in ['periodo', 'product_id', "tn"]]

# # Llenar valores faltantes para todas las características
# for feature in features:
#     data[feature] = data.groupby('product_id')[feature].ffill()

# # Normalizar las características para cada product_id
# scalers = {}
# for pid in data['product_id'].unique():
#     scaler = StandardScaler()
#     pid_data = data.loc[data['product_id'] == pid, features]
#     if not pid_data.empty:
#         data.loc[data['product_id'] == pid, features] = scaler.fit_transform(pid_data)
#         scalers[pid] = scaler

# # Función para crear secuencias que incluyan múltiples características y predigan dos meses en el futuro
# def create_sequences_for_two_months(data, n_steps, gap):
#     X, y = [], []
#     for i in range(len(data) - n_steps - gap):
#         X.append(data[i:(i + n_steps), :])
#         y.append(data[i + n_steps + gap, features.index('tn')])  # 'tn' es la característica incluida
#     return np.array(X), np.array(y)
...
# sequence_data = {}
# for pid in data['product_id'].unique():
#     df_temp = data[data['product_id'] == pid][features].values
#     if len(df_temp) > sequence_length + gap_months:
#         X, y = create_sequences_for_two_months(df_temp, sequence_length, gap_months)
#         sequence_data[pid] = (X, y)

# # Definir métrica personalizada Total Forecast Error (TFE)
# def total_forecast_error(y_true, y_pred):
#     return K.sum(K.abs(y_true - y_pred))

# # Definir el modelo
# model = Sequential([
#     Bidirectional(GRU(100, activation='relu', return_sequences=True, dropout=0.2, recurrent_dropout=0.2),
#                   input_shape=(sequence_length, len(features))),
#     Bidirectional(GRU(75, activation='relu', return_sequences=True, dropout=0.2, recurrent_dropout=0.2)),
#     Bidirectional(GRU(50, activation='relu', return_sequences=True, dropout=0.2, recurrent_dropout=0.2)),
#     Bidirectional(GRU(25, activation='relu', dropout=0.2, recurrent_dropout=0.2)),
#     Dense(1)
# ])

# custom_learning_rate = 0.0001
# optimizer = Adam(learning_rate=custom_learning_rate)
# model.compile(optimizer=optimizer, loss=total_forecast_error)

# # Función para predecir directamente el segundo mes
# def predecir_directamente_segundo_mes(data, mes_2):
#     predicciones = []

#     for pid in data['product_id'].unique():
#         if pid in sequence_data and len(sequence_data[pid][0]) > 0:
#             X, y = sequence_data[pid]
#             print(f"Entrenando modelo para product_id {pid}")
#             model.fit(X, y, epochs=20, verbose=0)

#             ultimo_periodo = data[data['product_id'] == pid].sort_values('periodo').iloc[-sequence_length:][features].values
#             ultimo_periodo = ultimo_periodo.reshape((1, sequence_length, len(features)))

#             prediccion_mes2_scaled = model.predict(ultimo_periodo)
#             print(f"Predicción escalada para {mes_2} product_id {pid}: {prediccion_mes2_scaled}")

#             prediccion_mes2_scaled_value = prediccion_mes2_scaled[0, 0]

#             zero_filled = np.zeros((1, len(features)))
#             zero_filled[0, features.index('tn')] = prediccion_mes2_scaled_value
#             prediccion_mes2 = scalers[pid].inverse_transform(zero_filled)[0, features.index('tn')]
#             print(f"Predicción desescalada para {mes_2} product_id {pid}: {prediccion_mes2}")

#         else:
#             if pid in productos_especificados:
#                 print(f"Usando promedio histórico para product_id {pid}")
#                 mean_tn = data[data['product_id'] == pid]['tn'].mean()
#                 prediccion_mes2 = mean_tn
#             else:
#                 continue
...
# # Hacer predicciones
# predicciones = predecir_directamente_segundo_mes(data, '2020-02')
